...
class HyperNet(AbstractHyperNet):
    ...
    def __init__(self,
        ...
        ## This one here is the codebooks
        ...
        ## This one can be completely replaced via a resonator network.
    ...
    def encode_properties(self, data: Data) -> Data:
        ...
        ## This is one call to the abstract encoder, same applies to the edge features and the graph features
    ...
    def forward(self,
        ...
        node_hv_stack[0] = data.node_hv # The initial node hv

        # ~ message passing
        for layer_index in range(self.depth):
            # messages are gated with the corresponding edge weights!
            messages = node_hv_stack[layer_index][dsts] * sigmoid(edge_weight)
            aggregated = scatter(messages, srcs, dim_size=node_hv_stack.shape[1], reduce='sum') # This is bundle
            #node_hv_stack[layer_index + 1] = normalize(self.bind_fn(node_hv_stack[0], aggregated))
            node_hv_stack[layer_index + 1] = normalize(self.bind_fn(node_hv_stack[layer_index], aggregated)) # this is obviously bind + normalise
        ...
        node_hv = node_hv_stack.sum(dim=0) # This is bundle -
    ...
    def possible_graph_from_constraints(self,
        ...
        # TODO: Ask what happened to the 'num'? Here we ignore the 'num' and make an edge between 'src' and 'dst' at all
        # occurrences if an has been decoded for between those edges
    ...
    def reconstruct(self,
        ...
        for _ in range(num_iterations):
            ...
            # TODO(Clarify): What if we don't have degree information? Is this optional?
