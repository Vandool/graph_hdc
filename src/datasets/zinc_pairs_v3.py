...
class ZincPairsV3(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.split = split
        self.cfg = cfg or ZincPairV3Config()
...
if __name__ == "__main__":
    ...
    pairs = ZincPairsV3(
        base_dataset=(ZincSmiles(split="test"))[:200],
        split="test",
        cfg=ZincPairV3Config(),  # use your current config
        dev=True,
    )
    ...
    for i, batch in enumerate(tqdm(loader, total=len(pairs), desc="VF2 sanity")):
        ...
        is_sub = is_induced_subgraph_by_features(
            DataTransformer.pyg_to_nx(g1),  # small / candidate
            DataTransformer.pyg_to_nx(g2),  # big / parent
        )
