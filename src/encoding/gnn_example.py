...
class ConditionalGraphAttention(MessagePassing):
    ...
    def forward(
        ...
        **kwargs,
        ...
        x = self.lay_transform_lin_1(torch.cat([node_embedding, x], axis=1), condition)
...
class ConditionalGIN(pl.LightningModule):
    ...
    def __init__(
        ...
        conv_units: list[int] = [64, 64, 64], # <-- solid base line, Depth is good (2 - 4), the units can be in range [32, 256]
        film_units: list[int] = [128],
        link_units: list[int] = [256, 64, 1],
        learning_rate: float = 0.001, # maybe start with 5e-4 or smaller
    ):
        """
        ...
        # TODO: for classification, the head should change
...
if __name__ == "__main__":
    ...
    nx.draw(
        G_vis, pos, with_labels=True, node_color="skyblue", node_size=500, edge_color=edge_colors, width=2, alpha=0.7
    )
