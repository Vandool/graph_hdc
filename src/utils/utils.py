...
class DataTransformer:
    ...
    def nx_to_pyg_with_type_attr(G: nx.Graph) -> Data:
        ...
        Similar to nx_to_pyg but expects node attribute 'type' instead of 'feat'.
        This is used for graphs generated by random matching algorithms.
        ...
        ------
        RuntimeError
            If PyTorch or PyTorch Geometric are not available
        """
        if torch is None or Data is None:
            raise RuntimeError("torch / torch_geometric are required for nx_to_pyg_with_type_attr")

        # Node ordering: use sorted ids for determinism
        nodes = sorted(G.nodes)
        idx_of: dict[int, int] = {n: i for i, n in enumerate(nodes)}

        # Features: extract from 'type' attribute
