...
from src.datasets.zinc_pairs_v2 import ZincPairsV2, is_induced_subgraph_feature_aware, pyg_to_nx, PairConfig
from src.datasets.zinc_smiles_generation import ZincSmiles


def compute_split_stats(ds: ZincPairsV2):
...
def summarize_split(ds: ZincPairsV2, name: str):
...
def sanity_check_split(pairs_ds: ZincPairsV2, split_name: str, n_samples: int = 500, seed: int = 0) -> None:
...
def run_pipeline_for_split(split_name: str, cfg: PairConfig, sanity_n: int, seed: int):
    print(f"\n=== Split: {split_name} ===")
    base = ZincSmiles(split=split_name)
    print(f"[{split_name}] base graphs: {len(base)}")

    pairs = ZincPairsV2(base_dataset=base, split=split_name, cfg=cfg)
