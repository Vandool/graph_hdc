...
# Model
# ---------------------------------------------------------------------
# two-tower with interaction head
class TwoTowerWithInteractions(nn.Module):
    ...
    def __init__(
        ...
        preproj_dim: int | None = None,   # e.g., 2048 to curb the first matmul
        freeze_preproj: bool = False,
        untie_towers: bool = False,       # set True if you want h1/h2 towers independent
    ):
        super().__init__()
...
def train(
    ...
    # ----- model + optim -----
    preproj_dim = 2048
    tower_dims = (2048, 1024, 256)
    head_dims = (512, 128)
    model = TwoTowerWithInteractions(
        hv_dim=cfg.hv_dim,
        preproj_dim=2048,  # None for the L config
        tower_dims=tower_dims,  # or (4096, 1024, 256) without preproj
        head_dims=head_dims,
        use_layernorm=True,
        untie_towers=False,
    ).to(device)
    ...
    base_row = _sanitize_for_parquet(
        {
            **asdict(cfg),
            ...
            "type": "TwinTowerWithInteractions",
            "model_dims_str": f"Pre-project dim: {preproj_dim}, Tower dim: {tower_dims}, Head dim: {head_dims}",
            "activation": "GELU",
            "dropout": 0.0,
            "normalization": "None",
        }
...
def run_experiment(cfg: Config):
    ...
    train(
        ...
        valid_pairs=train_small, # TODO: Change
        encoder=hypernet,
