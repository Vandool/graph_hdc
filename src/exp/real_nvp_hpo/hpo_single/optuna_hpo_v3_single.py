...
if __name__ == "__main__":
    ...
    study_name = f"{study_base}_v3_single_{ds.default_cfg.name}_{args.norm_per}"
    db_path = here / f"real_nvp_hpo_v3_single_{ds.default_cfg.name}_{args.norm_per}.db"
    csv = here / f"trials_v3_single_{ds.default_cfg.name}_{args.norm_per}.csv"

    print(f"Study name: {study_name}")
    print(f"Database: {db_path}")
    print(f"CSV export: {csv}")
    print("Model: NVP-V3 (semantic masking architecture)")
    print("Multi-objective minimize NLL, (2) minimize incorrect percentage (non-zero corrections)")
