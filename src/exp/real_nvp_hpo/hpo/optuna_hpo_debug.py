...
from src.exp.real_nvp_hpo.real_nvp_debug import run_qm9_trial
...
if __name__ == "__main__":
    ...
    study_name = f"{study_base}_{ds.default_cfg.name}_debug"
    db_path = here / f"real_nvp_hpo_{ds.default_cfg.name}_debug.db"
    csv = here / f"trials_{ds.default_cfg.name}_debug.csv"
    ...
    def objective(trial: optuna.Trial) -> float:
        val, decoded_edges_sizes = base_objective(trial, dataset=ds)
