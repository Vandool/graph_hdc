...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Real NVP V2 - HPO with Composite Metric (NLL + Validity)")
    p.add_argument(
        "--dataset",
        type=str,
    ...
    db_path = here / f"real_nvp_hpo_comp_{ds.default_cfg.name}_{args.norm_per}.db"
    csv = here / f"trials_comp_{ds.default_cfg.name}_{args.norm_per}.csv"
    ...
    # Choose an objective provided by your code (returns two objectives: NLL and 100-zero_correction_pct)
    ...
    def objective(trial: optuna.Trial) -> tuple[float, float]:
        try:
            min_nll, incorrect_pct = base_objective(trial, dataset=ds, norms_per=args.norm_per)
