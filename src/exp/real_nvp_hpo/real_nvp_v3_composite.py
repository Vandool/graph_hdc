...
class FlowConfig:
    ...
    # NVP-V3 specific hyperparameters
    num_flows: int = 4
    hidden_dim: int = 1024  # MLP hidden layer width
...
def run_experiment(cfg: FlowConfig):
    ...
    # ----- model / trainer -----
    # Use NVP-V3 model instead of NVP
    model = resolve_model("NVP-V3", cfg=cfg).to(device=device)
    ...
    best_model = retrieve_model("NVP-V3").load_from_checkpoint(best_path)
    ...
    # =================================================================
    # NEW: Decode samples and compute CorrectionLevel.ZERO for composite metric
    # =================================================================
    log("=== Starting CorrectionLevel.ZERO evaluation for composite metric ===")
...
def get_hidden_dim_dist(dataset: SupportedDataset):
    """Get hidden_dim ranges for NVP-V3 (different from num_hidden_channels in NVP)."""
    ...
    if dataset in [SupportedDataset.QM9_SMILES_HRR_256_F64_G1NG3, SupportedDataset.ZINC_SMILES_HRR_256_F64_5G1NG4]:
        ...
        low, high, step = 1024, 4096, 1024
    elif dataset == SupportedDataset.QM9_SMILES_HRR_1600:
        low, high, step = 800, 2400, 800
...
def run_zinc_trial(trial: optuna.Trial, dataset: SupportedDataset, norm_per: str):
    flow_cfg = get_cfg(trial, dataset=dataset)
    low, high, step = get_hidden_dim_dist(dataset)
    flow_cfg.hidden_dim = trial.suggest_int("hidden_dim", low, high, step=step)
    flow_cfg.num_hidden_layers = trial.suggest_int("num_hidden_layers", 2, 4)
    ...
    flow_cfg.per_term_standardization = norm_per == "term"
    flow_cfg.exp_dir_name = make_run_folder_name(
        {k: getattr(flow_cfg, k) for k in keys if k in flow_cfg.__dict__},
        prefix=f"nvp_v3_comp_{dataset.default_cfg.name}",
    )
    return run_experiment(flow_cfg)


def run_qm9_trial(trial: optuna.Trial, dataset: SupportedDataset, norm_per: str):
    flow_cfg = get_cfg(trial, dataset=dataset)
    low, high, step = get_hidden_dim_dist(dataset)
    flow_cfg.hidden_dim = trial.suggest_int("hidden_dim", low, high, step=step)
    flow_cfg.num_hidden_layers = trial.suggest_int("num_hidden_layers", 2, 4)
