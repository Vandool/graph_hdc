"""
Optuna HPO Script for Flow Matching with Single-Objective Optimization

This script uses fm_composite.py with single-objective optimization to minimize
the incorrect percentage (100 - CorrectionLevel.ZERO%).

Single-Objective Optimization:
Directly optimizes decoder quality (minimizing percentage of samples requiring corrections)
while tracking MSE as metadata for post-hoc analysis.

Usage:
======
# QM9 dataset (1000 decodes per trial)
python optuna_hpo_fm_single.py --dataset QM9_SMILES_HRR_1600_F64_G1NG3 --n_trials 50

# ZINC dataset (100 decodes per trial for efficiency)
python optuna_hpo_fm_single.py --dataset ZINC_SMILES_HRR_1024_F64_5G1NG4 --n_trials 100
python optuna_hpo_fm_single.py --dataset ZINC_SMILES_HRR_2048_F64_5G1NG4 --n_trials 100
"""
...
from src.exp.flow_matching.fm_composite import get_hidden_channel_dist, run_qm9_trial, run_zinc_trial
...
def get_space(dataset: SupportedDataset):
    ...
    return {
        ...
        "num_hidden_layers": optuna.distributions.IntDistribution(3, 8),
        "time_emb_dim": optuna.distributions.CategoricalDistribution([32, 64, 128]),
    }
...
def load_study(study_name: str, sqlite_path: str) -> optuna.Study:
    ...
    return optuna.create_study(
        ...
        direction=DIRECTION,  # Single-objective
...
def rebuild_study_from_csv(
    ...
    for _, r in df.iterrows():
        ...
        # Single-objective: read value (with backward compatibility for multi-objective CSV)
        if "value" in r and pd.notna(r["value"]):
            value = float(r["value"])
        elif "obj1_zero_correction_error" in r and pd.notna(r["obj1_zero_correction_error"]):
            # Backward compatibility: use obj1 from multi-objective runs
            value = float(r["obj1_zero_correction_error"])
            print(f"  Trial {int(r['number'])}: Using obj1_zero_correction_error={value:.2f}% from multi-objective CSV")
        else:
            value = None

        # Restore all user attributes from CSV
        # Skip standard columns: dataset, number, value/objectives, state, and hyperparameters
        standard_cols = {"dataset", "number", "value", "obj0_mse", "obj1_zero_correction_error", "state"}
        user_attrs = {}
        for col in r.index:
...
def export_trials(study_name: str, db_path: pathlib.Path, dataset: SupportedDataset, csv: pathlib.Path):
    ...
    Overwrites the dataset's CSV (no append/mix).
    """
    study = load_study(study_name, str(db_path))
    ...
    for t in study.get_trials(deepcopy=False):
        row = {
            ...
            "number": t.number,
            # Single-objective: store single value
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Flow Matching - HPO with Single Objective (Decoder Quality)")
    p.add_argument(
        "--dataset",
        type=str,
    ...
    # Paths (per-dataset DB + CSV) - use "_single" suffix to distinguish from multi-objective HPO
    here = pathlib.Path(__file__).parent
    study_base = here.parent.name
    study_name = f"{study_base}_single_{ds.default_cfg.name}_{args.norm_per}"
    db_path = here / f"fm_hpo_single_{ds.default_cfg.name}_{args.norm_per}.db"
    csv = here / f"trials_fm_single_{ds.default_cfg.name}_{args.norm_per}.csv"

    print(f"Study name: {study_name}")
    print(f"Database: {db_path}")
    print(f"CSV export: {csv}")
    print("Single-objective optimization: minimize incorrect percentage (non-zero corrections)")
    print("  MSE is tracked as metadata for post-hoc analysis")
    ...
    # Choose an objective provided by your code (returns two values: MSE and incorrect_pct)
    ...
    def objective(trial: optuna.Trial) -> float:
        try:
            ...
            # Training script saves to: src/exp/flow_matching/results/fm_composite/{exp_dir_name}/evaluations/hpo_metrics.json
            training_script_dir = here.parent  # Go up from hpo/ to flow_matching/
            ...
            if metrics_files:
                ...
                trial.set_user_attr("min_val_loss_ref", min_mse)  # Store MSE as reference (not optimized)
            ...
            # Return only incorrect_pct for single-objective optimization
    ...
    if study.best_trial:
        ...
        print(f"  Incorrect %: {study.best_value:.2f}%")
        print(f"  CorrectionLevel.ZERO %: {100.0 - study.best_value:.2f}%")
        if "min_val_loss_ref" in study.best_trial.user_attrs:
            print(f"  MSE (ref): {study.best_trial.user_attrs['min_val_loss_ref']:.6f}")
        if "exp_dir_name" in study.best_trial.user_attrs:
            print(f"  Experiment dir: {study.best_trial.user_attrs['exp_dir_name']}")
    ...
    if len(completed_trials) > 1:
        ...
        print(f"\nTop 5 trials (ranked by incorrect%):")
        for i, trial in enumerate(sorted_trials[:5], 1):
            incorrect_pct = trial.value
            zero_pct = 100.0 - incorrect_pct
            mse_ref = trial.user_attrs.get("min_val_loss_ref", "N/A")
            print(f"  {i}. Trial #{trial.number}: Incorrect%={incorrect_pct:.2f}%, ZERO%={zero_pct:.2f}%, MSE={mse_ref}")
