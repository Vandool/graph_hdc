"""
Optuna HPO Script for Flow Matching with Multi-Objective Optimization
...
allowing post-hoc selection of solutions based on preferred MSE/quality tradeoff.
...
def rebuild_study_from_csv(
    ...
    for _, r in df.iterrows():
        ...
        obj0 = float(r["obj0_mse"]) if "obj0_mse" in r and pd.notna(r["obj0_mse"]) else None
        obj1 = (
        ...
        standard_cols = {"dataset", "number", "obj0_mse", "obj1_zero_correction_error", "state"}
        user_attrs = {}
        for col in r.index:
...
def export_trials(study_name: str, db_path: pathlib.Path, dataset: SupportedDataset, csv: pathlib.Path):
    ...
    for t in study.get_trials(deepcopy=False):
        row = {
            ...
            "obj0_mse": t.values[0] if t.values else None,
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Real NVP V2 - HPO with Composite Metric (MSE + Validity)")
    p.add_argument(
        "--dataset",
        type=str,
    ...
    study_name = f"{study_base}_comp_{ds.default_cfg.name}_{args.norm_per}"
    db_path = here / f"fm_hpo_comp_{ds.default_cfg.name}_{args.norm_per}.db"
    csv = here / f"trials_fm_comp_{ds.default_cfg.name}_{args.norm_per}.csv"

    print(f"Study name: {study_name}")
    print(f"Database: {db_path}")
    print(f"CSV export: {csv}")
    print("Multi-objective optimization: (1) minimize MSE, (2) minimize incorrect percentage (non-zero corrections)")
    ...
    # Choose an objective provided by your code (returns two objectives: MSE and 100-zero_correction_pct)
    ...
    if study.best_trials:
        ...
        print("(Objectives: minimize MSE, minimize incorrect%; displayed as maximize CorrectionLevel.ZERO%)")
        for i, trial in enumerate(study.best_trials[:5], 1):
            mse = trial.values[0]
            incorrect_pct = trial.values[1]
            zero_pct = 100.0 - incorrect_pct
            print(f"  {i}. MSE: {mse:.4f}, Incorrect%: {incorrect_pct:.2f}%, CorrectionLevel.ZERO%: {zero_pct:.2f}%")
